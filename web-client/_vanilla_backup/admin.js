...
const navKb = document.querySelector("#navKb");
const navTrace = document.querySelector("#navTrace");
const traceDetailModal = document.querySelector("#traceDetailModal");
const closeTraceDetailModalBtn = document.querySelector("#closeTraceDetailModalBtn");
const traceDetailContent = document.querySelector("#traceDetailContent");
...
async function fetchTraceConversations(current = 1, size = 10) {
  const params = new URLSearchParams({ current: String(current), size: String(size) });
  return requestJson(`/api/admin/trace/conversations?${params.toString()}`, {
    method: "GET",
    headers: buildAuthHeaders()
  });
}

async function fetchTraceDetail(conversationId) {
  return requestJson(`/api/admin/trace/${encodeURIComponent(conversationId)}`, {
    method: "GET",
    headers: buildAuthHeaders()
  });
}
...
function renderBreadcrumb() {
  ...
  if (state.mode === ViewMode.TRACE) {
    const btn = document.createElement("button");
    btn.type = "button";
    btn.className = "crumb-btn active";
    btn.textContent = "链路追踪";
...
function renderTraceTable(records) {
  uploadSection.hidden = true;
  listSection.hidden = false;
  listTitle.textContent = "链路追踪列表";
  headerDesc.textContent = "查看每次对话的 Agent 工作流链路详情";
  primaryActionBtn.hidden = true;
  ...
  tableBody.innerHTML = "";
  if (!safeRecords.length) {
    tableBody.innerHTML = `<tr><td colspan="6">暂无链路记录</td></tr>`;
    return;
  }
  safeRecords.forEach((row) => {
    const tr = document.createElement("tr");
    tr.innerHTML = `
      <td>${escapeHtml(row.conversationId || "-")}</td>
      <td>${escapeHtml(String(row.nodeCount ?? 0))}</td>
      <td>${escapeHtml(row.startTime || "-")}</td>
      <td>${escapeHtml(row.endTime || "-")}</td>
      <td>${escapeHtml(String(row.totalDuration ?? 0))}</td>
      <td class="cell-actions">
        <button class="btn-action" data-action="trace-detail" data-conv-id="${escapeHtml(row.conversationId || "")}">查看详情</button>
      </td>
    `;
    tableBody.appendChild(tr);
  });
...
function renderTraceTimeline(detail) {
  if (!traceDetailContent) return;
  const steps = Array.isArray(detail?.steps) ? detail.steps : [];
  if (!steps.length) {
    traceDetailContent.innerHTML = `<p>暂无链路数据</p>`;
    return;
  }
  let html = `<div class="trace-timeline">`;
  steps.forEach((step, idx) => {
    const markerClass = step.status === "ERROR" ? "error" : step.status === "FINISH" ? "finish" : "start";
    const statusLabel = step.status === "ERROR" ? "异常" : step.status === "FINISH" ? "完成" : "开始";
    const durationText = step.duration != null ? `${step.duration}ms` : "-";
    html += `
      <div class="trace-step">
        <div class="trace-step-marker ${markerClass}"></div>
        <div class="trace-step-content">
          <div class="trace-step-header">
            <span class="trace-step-node">${escapeHtml(step.nodeName)}</span>
            <span class="status-badge ${markerClass === "error" ? "danger" : markerClass === "finish" ? "success" : "info"}">${statusLabel}</span>
            <span class="trace-step-time">${escapeHtml(durationText)}</span>
            <span class="trace-step-time">${escapeHtml(step.startTime || "-")}</span>
          </div>
          ${step.resultData ? `
            <details class="trace-step-detail">
              <summary>结果数据</summary>
              <pre class="trace-step-result">${escapeHtml(step.resultData)}</pre>
            </details>
          ` : ""}
          ${step.errorMessage ? `
            <div class="trace-step-error">
              <strong>错误:</strong> ${escapeHtml(step.errorMessage)}
            </div>
          ` : ""}
        </div>
      </div>
  ...
  html += `</div>`;
  traceDetailContent.innerHTML = html;
}

async function openTraceDetailModal(conversationId) {
  if (!traceDetailModal || !traceDetailContent) return;
  try {
    traceDetailContent.innerHTML = `<p>加载中...</p>`;
    traceDetailModal.hidden = false;
    const res = await fetchTraceDetail(conversationId);
    renderTraceTimeline(res.data);
    renderRawPayload(res);
  } catch (error) {
    traceDetailContent.innerHTML = `<p>加载失败：${serverErrorFallback(error)}</p>`;
...
async function loadPage(pageNo = 1) {
  ...
  try {
    if (state.mode === ViewMode.TRACE) {
      const res = await fetchTraceConversations(pageNo, state.page.size);
      const page = res.data || {};
      const records = page.records || [];
      renderRawPayload(res);
      updatePageMeta(page, records.length);
      renderTraceTable(records);
      return;
    }
...
tableBody.addEventListener("click", async (e) => {
  ...
  try {
    ...
    if (action === "trace-detail") {
      ...
      await openTraceDetailModal(convId);
      return;
    }
  } catch (error) {
    showToast(serverErrorFallback(error));
...
function updateNavActive() {
  if (navKb) navKb.classList.toggle("active", state.mode !== ViewMode.TRACE);
  if (navTrace) navTrace.classList.toggle("active", state.mode === ViewMode.TRACE);
}

navKb.addEventListener("click", () => {
  if (state.mode === ViewMode.TRACE) {
    state.mode = ViewMode.KB;
    state.selectedKb = null;
    state.selectedDoc = null;
...
navTrace.addEventListener("click", () => {
  if (state.mode !== ViewMode.TRACE) {
    state.mode = ViewMode.TRACE;
    loadPage(1);
  }
});
...
if (closeTraceDetailModalBtn) {
  closeTraceDetailModalBtn.addEventListener("click", () => {
    traceDetailModal.hidden = true;
  });
}
